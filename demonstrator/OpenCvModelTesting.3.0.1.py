...
chiffreDirectory="D:/Utilisateur/Documents/Isen_taff/projet_semestre_1/demonstrator4thewin/image_chiffre/"
...
while 1:
    ...
    if ((testTousHappy(listeEmotion)) & (timepicture - lastpicture > 5)):
        ...
        if(time.time()-time_tampon<1):
            print(np.shape(img),np.shape(liste_npChiffre[0]))
            overlay_image_alpha(img, liste_npChiffre[0][:,:,:3], (200,200),liste_npChiffre[0][:,:,3]/255.0 )
            #superpose(img, liste_npChiffre[0], 0,0)
        elif((time.time()-time_tampon>1) & (time.time()-time_tampon<2) ):
            overlay_image_alpha(img, liste_npChiffre[1][:,:,:3], (200,200),liste_npChiffre[1][:,:,3]/255.0 )
            #superpose(img, liste_npChiffre[1], 0, 0)
        elif((time.time() -time_tampon> 2) & (time.time()-time_tampon< 3)):
            overlay_image_alpha(img, liste_npChiffre[2][:, :, :3], (200, 200), liste_npChiffre[2][:, :, 3] / 255.0)
