...
def generate_arrays_from_file(images, labels, batch_size=32, shuffle=True):
    ...
    while 1:
        for i in idx:
            x[batch_id, ...] = images[i]
            y[batch_id, ...] = keras.utils.to_categorical(labels[i], num_classes=4)
...
def tcdcn_2d():
    ...
    model.add(LocallyConnected2D(16, (5, 5), padding='valid', input_shape=(49, 49, 3)))
...
if __name__ == "__main__":
    ...
    checkpointer = ModelCheckpoint(filepath='snapshots/tcdcn-{epoch:03d}-{val_loss:.6f}.h5', verbose=1, save_best_only=True)
    hist = model.fit_generator(generate_arrays_from_file(images_training, annotations_training), int(48000*4/32), epochs=500, callbacks=[csv_logger, checkpointer], validation_data=generate_arrays_from_file(images_validation, annotations_validation, shuffle=False), validation_steps=int(12000*4/32), max_queue_size=1, workers=1, use_multiprocessing=False, initial_epoch=0)
